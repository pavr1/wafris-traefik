...
func LoadUserDefinedProxies() {
	...
	for _, udp_str := range udps {
		...
		if len(normalized_udp_str) > 0 {
			...
			if err == nil {
				log.Println(9358424034, "wafris-caddy regexp added for trusted proxy: ", udp_str)
				userDefinedProxiesRegexs = append(userDefinedProxiesRegexs, compliled_udp)
			} else {
				log.Println(9358424035, "wafris-caddy regexp failed to compile: ", compliled_udp, err)
			}
		}
	}
}
